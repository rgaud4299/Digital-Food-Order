...
const { getDayUTC, ISTFormat, ISTDate } = require("../../utils/helper");
...
//     console.log("start_dateend_date", start_date, end_date, ISTDate())
...
//           table: { select: { table_number: true, location: true } },
//           order_items: {
//             include: {
...
//       table_details: order.restaurant_tables
//         ? {
//           table_number: order.restaurant_tables.table_number,
//           location: order.restaurant_tables?.location || null
//         }
//         : null,
...
const getOrderList = async (req, res) => {
  try {
    ...
    if (customer) {
      roleWhere.push({
        customer_id: BigInt(customer.customer_id)
      });
    }

    // 🏪 RESTAURANT ADMIN
    else if (user) {
      if (["PlatformAdmin", "RestaurantStaff"].includes(user?.role)) {
        roleWhere.push({
          restaurant_id: BigInt(user.restaurant_id)
        });
      }
    }
    ...
    const where = {
      AND: [
        ...
        order_no
          ? { order_no: { contains: order_no, mode: "insensitive" } }
          : null,
    ...
    const [total, filteredCount, orders] = await Promise.all([
      ...
      prisma.orders.findMany({
        ...
        include: {
          ...
          customer: {
            select: { uuid: true, name: true, mobile_no: true }
          },
          table: {
            select: { table_number: true, location: true }
          },
          order_items: {
            include: {
              food_items: { select: { id: true, name: true } },
